...
# Add the project root to the path to import MongoEngine models
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))

try:
    from students.models_mongo import (
        Department, Batch, Student, StudentBacklog, 
        StudentMentor, StudentNote, Attendance
    )
    MONGODB_AVAILABLE = True
except ImportError:
    MONGODB_AVAILABLE = False
    print("MongoDB models not available. Using regular Django models.")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if MONGODB_AVAILABLE:
            self.create_mongodb_data(options)
        else:
            self.create_django_data(options)

    def create_mongodb_data(self, options):
        """Create sample data using MongoDB models"""
        
        if options['clear']:
            self.stdout.write('Clearing existing MongoDB data...')
            Student.drop_collection()
            Department.drop_collection()
            Batch.drop_collection()
            Attendance.drop_collection()
            StudentBacklog.drop_collection()
            StudentMentor.drop_collection()
            StudentNote.drop_collection()
        ...
        for i in range(options['students']):
            ...
            student = Student(
                ...
                enrollment_date=date(batch.year, random.randint(6, 8), random.randint(1, 28)),
            )
            student.save()
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully created sample data in MongoDB:\n'
    ...
    def create_django_data(self, options):
        """Fallback to Django ORM if MongoDB is not available"""
        self.stdout.write('Using Django ORM fallback...')
        # Implementation would be similar but using Django ORM methods